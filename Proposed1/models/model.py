...
# =========================================================
#  Hard-coded Stage1 checkpoint path (fixed as requested)
# =========================================================
STAGE1_CKPT_PATH = "checkpoint/richcount_stage1_fsc147.pt"

# =========================================================
#  Stage1 Heads: FFN & Adapter (same as your Stage1)
# =========================================================
...
def _load_stage1_heads_into(ffn: ImageFFN, adapter: TextAdapter, ckpt_path: str, device: torch.device):
    if not os.path.exists(ckpt_path):
        raise FileNotFoundError(
            f"Stage1 checkpoint not found: {ckpt_path}\n"
            f"Please put your trained checkpoint at exactly this path."
        )

    ckpt = torch.load(ckpt_path, map_location="cpu")
    sd = ckpt["state_dict"] if "state_dict" in ckpt else ckpt

    ffn_sd = {k.replace("ffn.", ""): v for k, v in sd.items() if k.startswith("ffn.")}
    adp_sd = {k.replace("adapter.", ""): v for k, v in sd.items() if k.startswith("adapter.")}

    # Strict True to ensure correct weights
    ffn.load_state_dict(ffn_sd, strict=True)
    adapter.load_state_dict(adp_sd, strict=True)
...
class Proposed1Count(nn.Module):
    def __init__(self, fim_depth: int = 4,
        ...
        # --------------------------------------------------------------------------
        # Stage1 heads (hard-coded load). Only valid for dim=512 (b16/b32).
        self.use_stage1 = True
        if self.use_stage1:
            if self.clip_out_dim != 512:
                raise ValueError(
                    f"Stage1 checkpoint at {STAGE1_CKPT_PATH} is for dim=512 (ViT-B/16 or ViT-B/32). "
                    f"Current backbone={backbone} has clip_out_dim={self.clip_out_dim}."
                )
            self.stage1_ffn = ImageFFN(dim=512, hidden_dim=512)
            self.stage1_adapter = TextAdapter(dim=512, hidden_dim=512)
            _load_stage1_heads_into(self.stage1_ffn, self.stage1_adapter, STAGE1_CKPT_PATH, device=torch.device("cuda"))
        ...
        self.patch_emb_pos_embed = nn.Parameter(torch.zeros(1, n_token, self.clip_out_dim),
        ...
        # --------------------------------------------------------------------------
    # ----------------------------  
    # Stage1 helper encoders
    # ----------------------------
    @torch.no_grad()
    def _encode_text_stage1(self, token_2d: torch.Tensor) -> torch.Tensor:
        """
        token_2d: (N, 77)
        returns: (N, 1, 512) normalized
        """
        base = self.clip.encode_text(token_2d).float()         # (N,512)
        base = F.normalize(base, dim=-1)
        out = self.stage1_adapter(base)                        # (N,512)
        out = F.normalize(out, dim=-1)
        return out.unsqueeze(1)                                # (N,1,512)

    @torch.no_grad()
    def _align_cls_stage1(self, cls_token: torch.Tensor) -> torch.Tensor:
        """
        cls_token: (B,1,512)
        returns: (B,1,512) aligned + normalized
        """
        cls = cls_token.squeeze(1).float()                     # (B,512)
        cls = F.normalize(cls, dim=-1)
        cls = self.stage1_ffn(cls)                             # (B,512)
        cls = F.normalize(cls, dim=-1)
    ...
    def forward_decoder(self, cls_token, img_feat_patches, gt_rank_text_embedding):
        ...
        y_ = gt_rank_text_embedding  # [B, 1, 512]
    ...
    def forward(self, imgs, class_text, current_epoch, top_fine_text_embedding, return_extra: bool = False,
        ...
        if self.use_stage1:
            with torch.no_grad():
                class_text_embedding = self._encode_text_stage1(class_text_token).float()  # (B*12,1,512)
        else:
        ...
        if self.use_stage1:
            cls_token = self._align_cls_stage1(cls_token)
        patch_feat = img_feat_patches[:, 1:, :]
        patch_embedding_contrast = self.patch_feat_proj_contrast(patch_feat)
        if self.use_stage1:
            top_fine_text_embedding = F.normalize(top_fine_text_embedding.float(), dim=-1)
        
        # pred_density = self.forward_decoder(img_feat_patches, top_fine_text_embedding) 

        pred_density = self.forward_decoder(cls_token, img_feat_patches, top_fine_text_embedding)
    ...
    def test_forward(self, imgs, rank_text_token, coop_require_grad):
        ...
        cls_token, img_feat_patches = self.forward_visual_encoder(imgs, text_embedding=None)
        pred_density, extra_out = self.forward_decoder(img_feat_patches, rank_text_embedding, rank_text_embedding,
                                                       cls_token)  # [N, 384, 384]
        return pred_density, extra_out
    ...
    def validation_forward(self, imgs, rank_text_list, is_fine=False):
        ...
        with torch.no_grad():
            if self.use_stage1:
                rank_text_embedding = self._encode_text_stage1(rank_text_token).float()  # (B*?,1,512)
            else:
                rank_text_embedding = self.text_encoder(rank_text_token).float()
        ...
        if is_fine:
            ...
            pred_density = self.forward_decoder(cls_token, img_feat_patches, gt_rank_text_embedding)

        return top_indices, pred_density

    def train_forward(self, imgs, rank_text_list, index_GT, is_fine=False):
        rank_text_token = clip.tokenize(rank_text_list).to(imgs.device)

        if self.use_stage1:
            rank_text_embedding = self._encode_text_stage1(rank_text_token).float()
        else:
            rank_text_embedding = self.text_encoder(rank_text_token).float()
...
class CLIPViT(nn.Module):
    ...
    def __init__(self,
                 ...
                 vpt_depth: int = 8,
                 unfreeze: bool = False,
                 contrast_pre_epoch: int = 20) -> None:
        """
        Param:
        ...
        for i in range(transformer.layers):
            if i == 0:
                ...
                deep_prompt_emb = self.vpt_dropout(
                    self.vpt_proj(self.visual_prompt[i - 1, ...]).expand(B, -1, -1)).permute(1, 0, 2)
                # B, L, 768
...
class CLIPTextTransformer(nn.Module):
    ...
    def __init__(self, clip_model, use_coop: bool, n_ctx: int = 2, contrast_pre_epoch: int = 20) -> None:
        super().__init__()
