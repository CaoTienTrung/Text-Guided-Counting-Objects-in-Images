...
class ConvCrossAttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        """
        Building block of the hierarchical vision trasformer for patch-text interaction
    ...
    def forward(self, x, y, cls_token):
        if self.resolution != 1:
            ...
            x = self._2d_to_seq(x)
        x = x + self.drop_path0(self.selfattn(self.norm0(x)))
        x = x + self.drop_path1(self.attn(self.norm1(cls_token), y))
        x = x + self.drop_path2(self.mlp(self.norm2(x)))
        return x
    ...
    def seq_to_2d(self,x):
        ...
        x = x.transpose(1, 2).reshape(n, c, h, w) #pixel-text matching map
        return x
